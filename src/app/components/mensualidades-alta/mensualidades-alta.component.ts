...
export class MensualidadesAltaComponent implements OnInit {
  ...
  setService(event) {
    ...
    const servicio = this.tarifas.filter(tarifa => tarifa.id == event.target.value);
  ...
  setDates(event) {
    ...
    this.membresia.f_corte = fAlta.add(1, 'months').subtract(1, 'days').valueOf();
  }
  ...
  sendMembresia() {
    ...
    this.serviciosService.postMembresia(this.membresia).subscribe(
      response => {
        alert(`${response.message} \n Membresia registrada exitosamente`)
        this.formato.reset()
      },
      error => {
        console.log(error)
